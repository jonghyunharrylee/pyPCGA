...
if __name__ == '__main__':
	...
	dim = 3
	N = np.array([30, 10, 10])
	#theta = np.ones((dim), dtype='d')
	theta = np.array([100.,100.,10.])
	xmin = np.array([0,0,0])
	xmax = np.array([300.,100.,20.])
	row, pts = CreateRow(xmin, xmax, N, kernel,theta)
	n = pts.shape

	v1 = np.ones((3000,1))
	res1 = ToeplitzProduct(v1, row, N)
	print(res1)

	v2 = np.sin(np.linspace(1,3000,3000))
	res2 = ToeplitzProduct(v2, row, N)
	print(res2)

	nsize = 3000
	def mv(vv):
		matvec = ToeplitzProduct(vv, row, N)
		return matvec

	from scipy.sparse.linalg import eigsh
	from scipy.sparse.linalg import LinearOperator # Matrix-free IterativeSolve

	Q = LinearOperator((nsize, nsize), matvec=mv, rmatvec=mv, dtype='d')
